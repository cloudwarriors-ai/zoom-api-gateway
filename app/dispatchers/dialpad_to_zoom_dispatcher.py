"""
Dialpad to Zoom Transformation Dispatcher

This dispatcher handles all Dialpad to Zoom transformations by routing
...
class DialpadToZoomDispatcher:
    """Dispatcher for Dialpad to Zoom transformations."""
    ...
    def _initialize_transformers(self):
        ...
        try:
            # Import transformers
            from app.transformers.raw_to_zoom.dialpad_to_zoom.sites_transformer import DialpadSitesToZoomTransformer
            from app.transformers.raw_to_zoom.dialpad_to_zoom.users_transformer import DialpadUsersToZoomTransformer
            from app.transformers.raw_to_zoom.dialpad_to_zoom.call_queues_transformer import DialpadCallQueuesToZoomTransformer
            ...
            from app.transformers.raw_to_zoom.dialpad_to_zoom.ivr_transformer import DialpadIvrToZoomTransformer
            
            # Map job type codes to transformer instances
            sites_transformer = DialpadSitesToZoomTransformer(job_type_code="dialpad_zoom_sites")
            users_transformer = DialpadUsersToZoomTransformer(job_type_code="dialpad_zoom_users")
            call_queues_transformer = DialpadCallQueuesToZoomTransformer(job_type_code="dialpad_zoom_callqueues")
            auto_receptionists_transformer = DialpadAutoReceptionistsToZoomTransformer(job_type_code="dialpad_zoom_autoreceptionists")
            ivr_transformer = DialpadIvrToZoomTransformer(job_type_code="dialpad_zoom_ivr")
            
            self._transformers = {
                # Standard format (dialpad_zoom_*)
                "dialpad_zoom_sites": sites_transformer,
                "dialpad_zoom_users": users_transformer,
                "dialpad_zoom_callqueues": call_queues_transformer,
                "dialpad_zoom_autoreceptionists": auto_receptionists_transformer,
                "dialpad_zoom_ivr": ivr_transformer,
            }
            
            logger.info(f"Initialized Dialpad to Zoom dispatcher with {len(self._transformers)} transformers")
            
        except Exception as e:
            logger.error(f"Failed to initialize Dialpad to Zoom transformers: {str(e)}")
    ...
    def transform(self, job_type_code: str, raw_data: Dict[str, Any], job: Optional[Any] = None) -> Dict[str, Any]:
        ...
        Args:
            job_type_code: The job type code (e.g., 'dialpad_zoom_sites')
        ...
        if not self.supports_job_type(job_type_code):
            ...
            raise TransformationError(
                f"Unsupported job type '{job_type_code}' for Dialpad to Zoom transformation. "
